...
/// Sparses 3D-Dichteraster fuer Hoehlenrauschen: Rauschen wird nur an `stride`-Abstaenden
/// ausgewertet, dazwischen wird trilinear interpoliert. Mindest-Stride 2 (Stack-Bedarf waechst
/// kubisch: stride=1 waere 33^3*4B = 143 KiB pro Chunk). Anders als eine Hoehenkarte variiert die
/// Hoehlendichte tatsaechlich in allen 3 Dimensionen, daher bleibt der Rand-Shell-Exaktheits-Trick
/// (s. `TerrainGenerator::generate_chunk`) hier ein echter Gewinn statt auf die volle Flaeche zu
/// entarten.
pub struct CaveGrid {
