...
/// Zylindrisches Billboard: die Basis/Spitze-Achse ist fix vertikal (identisch zur Spitzen-Position,
/// die der Compute-Pass fuer die Kantenerkennung nutzt - Render und Sampling muessen denselben Punkt
/// meinen), nur die Breiten-Achse rotiert um diese Achse zur Kamera. Die Spitze wird zusaetzlich
/// leicht entlang der Sonnen-XZ-Richtung geschert, damit der Strahl sichtbar von der Sonne weg zu
/// "strahlen" scheint statt starr senkrecht zu stehen.
@vertex
fn vs_main(@builtin(vertex_index) vertex_index: u32, @builtin(instance_index) instance_index: u32) -> VertexOutput {
    ...
    let height = ray.size.y;

    let view_dir = normalize(u.camera_pos.xyz - base);
    let axis_up = vec3<f32>(0.0, 1.0, 0.0);
    var right = cross(axis_up, view_dir);
    if dot(right, right) < 1e-6 {
        right = vec3<f32>(1.0, 0.0, 0.0);
    }
    right = normalize(right) * width * 0.5;

    let sun_xz_raw = vec3<f32>(u.sun_direction_to_sun.x, 0.0, u.sun_direction_to_sun.z);
    let sun_xz = select(normalize(sun_xz_raw), vec3<f32>(1.0, 0.0, 0.0), dot(sun_xz_raw, sun_xz_raw) < 1e-6);
    let shear = -sun_xz * height * 0.3;

    let corner = CORNER_OFFSETS[vertex_index % 6u];
    let side = (corner.x * 2.0 - 1.0) * right;
    let world_pos = base + axis_up * height * corner.y + shear * corner.y + side;
...
fn fs_main(in: VertexOutput) -> @location(0) vec4<f32> {
    ...
    // "Nach unten faellt der Strahl ab": hell an der Spitze (uv.y = 1, nahe der Lichtquelle),
    // verblasst zur Basis (uv.y = 0).
    let vertical_fade = in.uv.y;
